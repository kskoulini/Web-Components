const dltTextInputTemplate: HTMLTemplateElement = document.createElement('template');
...
class dltTextInput extends HTMLElement {
    ...
    clearInputField() {
        let textInput : HTMLInputElement|null|undefined = this.shadowRoot?.querySelector('.dlt-text-input');
    ...
    connectedCallback(){
        ...
        let formItem : HTMLElement|null|undefined = this.shadowRoot?.querySelector('.dlt-form-item');
        let textInput : HTMLElement|null|undefined = this.shadowRoot?.querySelector('.dlt-text-input');
        let formLabel : HTMLElement|null|undefined = this.shadowRoot?.querySelector('.dlt-form-label');
        let clearIcon : HTMLElement|null|undefined = this.shadowRoot?.querySelector('.dlt-form-group .clear-icon');

        // Adding the user defined classes
        [...this.classList].forEach(cn => {
            formItem?.classList.add(cn);
        })
        ...
        if(this.hasAttribute('placeholder')){
            textInput?.setAttribute('placeholder',this.getAttribute('placeholder') || 'mary@softwareaf.com');
        }

        // Handling error state if specified by 'error-input' class
        if(this.classList.contains('error-input')){
            textInput?.setAttribute('aria-invalid','true');
            textInput?.setAttribute('aria-describeby','text-error');
        ...
        clearIcon?.addEventListener("click", this.clearInputField);
        
    }

    disconnectedCallback() {
        let clearIcon : HTMLElement|null|undefined = this.shadowRoot?.querySelector('.dlt-form-group .clear-icon');
        clearIcon?.removeEventListener('click',this.clearInputField);
      }
